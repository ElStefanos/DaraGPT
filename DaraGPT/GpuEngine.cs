                ...
                foreach (var plat in platforms)
                {
                    ...
                    foreach (var dev in devices)
                    {
                            ...
                            platName.ToUpper().Contains(preferVendor.ToUpper()))
                        {
                            ...
                            Queue = Cl.CreateCommandQueue(Context, Device, (CommandQueueProperties)0, out err);
                            Available = true;
                            BuildProgram();
...
// === MatMul koji stvarno poziva tvoja C# funkcija ===
...
// === Stabilan row-wise softmax koji tvoja C# funkcija traži ===
...
";
        ...
        public float[] MatMul(float[] A, float[] B, int M, int N, int P)
        {
            ...
            var err = ErrorCode.Success;
            var bufA = Cl.CreateBuffer(Context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, A.Length * sizeof(float), A, out err);
            var bufB = Cl.CreateBuffer(Context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, B.Length * sizeof(float), B, out err);
            ...
            var global = new IntPtr[] { (IntPtr)M, (IntPtr)P };
            err |= Cl.EnqueueNDRangeKernel(Queue, kernel, 2, null, global, null, 0, null, out _);

            var result = new float[M * P];
            err |= Cl.EnqueueReadBuffer(Queue, bufOut, Bool.True, IntPtr.Zero, new IntPtr(result.Length * sizeof(float)), result, 0, null, out _);
            Cl.Finish(Queue);
            ...
            float[] GradW, float[] GradB, float[] GradIn)
        {
            ...
            var bufX = Cl.CreateBuffer(Context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, X.Length * sizeof(float), X, out err);
            var bufW = Cl.CreateBuffer(Context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, W.Length * sizeof(float), W, out err);
            var bufGO = Cl.CreateBuffer(Context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, GradOut.Length * sizeof(float), GradOut, out err);
            ...
            var globalW = new IntPtr[] { (IntPtr)Out, (IntPtr)In };
            ...
            var globalB = new IntPtr[] { (IntPtr)Out };
            ...
            var globalIn = new IntPtr[] { (IntPtr)B, (IntPtr)In };
            Cl.EnqueueNDRangeKernel(Queue, kIn, 2, null, globalIn, null, 0, null, out _);
            Cl.ReleaseKernel(kIn);

            Cl.EnqueueReadBuffer(Queue, bufGradW, Bool.True, IntPtr.Zero, new IntPtr(GradW.Length * sizeof(float)), GradW, 0, null, out _);
            Cl.EnqueueReadBuffer(Queue, bufGradB, Bool.True, IntPtr.Zero, new IntPtr(GradB.Length * sizeof(float)), GradB, 0, null, out _);
            Cl.EnqueueReadBuffer(Queue, bufGradIn, Bool.True, IntPtr.Zero, new IntPtr(GradIn.Length * sizeof(float)), GradIn, 0, null, out _);
        ...
        public float[] RowSoftmax(float[] X, int rows, int cols)
        {
            ...
            var global = new IntPtr[] { (IntPtr)rows };
        ...
        public float[] UpdateWeights(float[] weights, float[] grads, float lr)
        {
            ...
            var global = new IntPtr[] { (IntPtr)weights.Length };
